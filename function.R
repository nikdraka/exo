...
matrixMaker <- function(x0, y, type = c("optimized", "backcasting")) {
  ...
  if (type == "optimized") {
    ...
    init <- c(unname(coef(lm(head(y,freq.ts)~seq(1,freq.ts,1)))))
  }
  
  return(list(measurementMatrix = matH,
              transitionMatrix = matF,
...
gen.forecast <- function(x, statevec = statevector, h) {
  ...
  f0 <- tail(statevec[-nrow(statevec),], 1)
  seq.h <- seq(1,h)
  
  f <- f0[1] + f0[2]*seq.h*(x[3]^(seq.h))
  ...
  if (type == "optimized") {
    ...
    for (j in 1:10) {
      ...
      for (i in 1:nTrain) {
        ...
        statevec[i,] <- l1
        l0 <- l1
        # l0 <- rbind(statevec[,1], x0[3]*statevec[i,2])
      }
      s0 <- rbind(statevec[1,1], matF[1,2]*statevec[1,2])
    }
...
fun.iter <- function(sq = seq(0, 1, 0.1)) {
  ...
  bm.optimal <- loopLambdaCV(y, type = "optimized", penalty = "lasso", sq = 0)
  bm.backcasting <- loopLambdaCV(y, type = "backcasting", penalty = "lasso", sq = 0)
  ...
  # return(list(cbind(bm.ets, coba.lasso.optimal),
  #             cbind(bm.ets, coba.lasso.backcast),
  #             cbind(bm.ets, coba.adapt.optimal),
  #             cbind(bm.ets, coba.adapt.backcast)))
  
  return(list(bm.optimal, 
              bm.backcasting,
              coba.lasso.optimal,
              coba.lasso.backcast,
              coba.adapt.optimal,
...
system.time({xyz <- fun.iter(sq=sqq)})
